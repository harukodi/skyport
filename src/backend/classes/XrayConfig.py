...
class XrayConfig:
    ...
    def _generate_xray_qr_code_and_vless_link(self):
        ...
        vless_uri = f"vless://{self.xray_uuid}@{self.domain_name}:{self.port}?encryption={self.xray_encryption_key}&flow=xtls-rprx-vision&security=tls&sni={self.domain_name}&fp=chrome&alpn=h3%2Ch2%2Chttp%2F1.1&type=xhttp&host={self.domain_name}&path={self.xray_path}&mode=auto#{encoded_remark}"
